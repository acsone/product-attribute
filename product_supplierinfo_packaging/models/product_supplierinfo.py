...
class ProductSupplierInfo(models.Model):
    ...
    def _inverse_packaging_min_qty(self):
        ...
        if not packaging:
            return
        packaging_uom = packaging.product_uom_id
        uom = self.product_uom or self.product_tmpl_id.uom_id
        packaging_uom_qty = packaging_uom._compute_quantity(self.packaging_min_qty, uom)
        packaging_qty = float_round(
            packaging_uom_qty * packaging.qty, precision_rounding=packaging_uom.rounding
        )
        self.min_qty = packaging_qty
